...
pub async fn get_accounts(db: Db) -> Result<Vec<models::account::AccountGeneral>, rusqlite::Error> {
    tracing::info!("Invocation to `get_accounts`");
    let conn = db.lock().unwrap(); // make generic error?
    let mut stmt = conn
        .prepare("SELECT account_number, user_id, created_at FROM ACCOUNTS;")
        .unwrap();
...
) -> Result<models::account::AccountCreation, rusqlite::Error> {
